...
class MusicUtils {
  ...
  /// 简谱基础数字（不含升降号）
  static const jianpuBaseNumbers = [
    '1',
    '1',
    '2',
    '2',
  ...
  static int jianpuStringToMidi(String jianpu, {int baseOctave = 4}) {
    // 统计高低八度标记（支持两种格式）
    final highOctaves =
        "'".allMatches(jianpu).length + _highDot.allMatches(jianpu).length;
    final lowOctaves =
        ",".allMatches(jianpu).length + _lowDot.allMatches(jianpu).length;
  ...
  static String formatJianpuDisplay(String jianpu) {
    ...
    while (result.contains("'")) {
      ...
      if (index > 0) {
        // 在数字后插入上加点
        result =
            result.substring(0, index) + _highDot + result.substring(index + 1);
      } else {
        break;
      }
    }
    ...
    while (result.contains(",")) {
      ...
      if (index > 0) {
        // 在数字后插入下加点
        result =
            result.substring(0, index) + _lowDot + result.substring(index + 1);
      } else {
        break;
      }
    }
