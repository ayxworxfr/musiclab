# 乐理通 - 应用架构设计文档

## 一、架构设计目标

### 1.1 设计原则

| 原则 | 说明 | 实践方式 |
|------|------|---------|
| **单一职责** | 每个模块/类只负责一件事 | 按功能拆分模块，Controller 只处理业务逻辑 |
| **开闭原则** | 对扩展开放，对修改关闭 | 使用抽象接口，依赖注入 |
| **依赖倒置** | 依赖抽象而非具体实现 | Repository 模式，接口隔离 |
| **关注点分离** | UI、业务逻辑、数据分离 | 三层架构 |
| **DRY** | 不要重复自己 | 抽取公共组件和工具类 |

### 1.2 质量目标

| 目标 | 指标 | 实现方式 |
|------|------|---------|
| **可维护性** | 修改代码不影响其他模块 | 模块化、低耦合 |
| **可扩展性** | 新增功能无需大改动 | 插件化架构、依赖注入 |
| **可测试性** | 核心逻辑可单元测试 | 依赖注入、Mock |
| **可读性** | 代码结构清晰易懂 | 规范命名、完善注释 |
| **性能** | 流畅的用户体验 | 懒加载、缓存策略 |

---

## 二、整体架构

### 2.1 分层架构图

```
┌─────────────────────────────────────────────────────────────────────┐
│                          Presentation Layer                          │
│                            (表现层/UI层)                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐                  │
│  │    Pages    │  │   Widgets   │  │   Dialogs   │                  │
│  │    页面     │  │    组件     │  │   弹窗      │                  │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘                  │
│         │                │                │                          │
│         └────────────────┼────────────────┘                          │
│                          │                                           │
│                          ▼                                           │
├─────────────────────────────────────────────────────────────────────┤
│                          Business Layer                              │
│                          (业务逻辑层)                                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐                  │
│  │ Controllers │  │   States    │  │   Events    │                  │
│  │   控制器    │  │    状态     │  │    事件     │                  │
│  └──────┬──────┘  └─────────────┘  └─────────────┘                  │
│         │                                                            │
│         ▼                                                            │
├─────────────────────────────────────────────────────────────────────┤
│                          Domain Layer                                │
│                          (领域层/服务层)                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐                  │
│  │  Services   │  │    Models   │  │ Repositories│                  │
│  │    服务     │  │    模型     │  │   仓储接口   │                  │
│  └──────┬──────┘  └─────────────┘  └──────┬──────┘                  │
│         │                                 │                          │
│         ▼                                 ▼                          │
├─────────────────────────────────────────────────────────────────────┤
│                          Data Layer                                  │
│                          (数据层)                                    │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐                  │
│  │  Network    │  │   Storage   │  │    Cache    │                  │
│  │  网络请求   │  │  本地存储   │  │    缓存     │                  │
│  └─────────────┘  └─────────────┘  └─────────────┘                  │
└─────────────────────────────────────────────────────────────────────┘
```

### 2.2 各层职责

| 层级 | 职责 | 包含内容 |
|------|------|---------|
| **表现层** | 用户界面展示、用户交互 | Pages、Widgets、动画、路由 |
| **业务层** | 业务逻辑处理、状态管理 | Controllers、状态对象 |
| **领域层** | 核心业务规则、领域模型 | Services、Models、Repository接口 |
| **数据层** | 数据获取与存储 | API调用、本地存储、缓存 |

---

## 三、项目目录结构

```
musiclag/
├── lib/
│   ├── main.dart                           # 应用入口
│   │
│   ├── app/                                # 📱 应用配置层
│   │   ├── app.dart                       # MaterialApp 配置
│   │   ├── app_binding.dart               # 全局依赖注入
│   │   ├── routes/                        # 路由管理
│   │   │   ├── app_routes.dart           # 路由名称常量
│   │   │   └── app_pages.dart            # 路由页面映射
│   │   ├── middlewares/                   # 路由中间件
│   │   │   └── auth_middleware.dart
│   │   └── config/                        # 应用配置
│   │       ├── app_config.dart           # 环境配置
│   │       └── api_config.dart           # API 配置
│   │
│   ├── core/                               # 🔧 核心基础层（业务无关）
│   │   ├── base/                          # 基类
│   │   │   ├── base_controller.dart      # Controller 基类
│   │   │   ├── base_page.dart            # Page 基类
│   │   │   └── base_repository.dart      # Repository 基类
│   │   │
│   │   ├── network/                       # 网络层
│   │   │   ├── http_client.dart          # Dio 封装
│   │   │   ├── api_response.dart         # 响应模型
│   │   │   ├── api_exception.dart        # 异常定义
│   │   │   └── interceptors/             # 拦截器
│   │   │       ├── auth_interceptor.dart
│   │   │       ├── log_interceptor.dart
│   │   │       └── cache_interceptor.dart
│   │   │
│   │   ├── storage/                       # 存储层
│   │   │   ├── storage_service.dart      # 存储服务接口
│   │   │   ├── local_storage.dart        # SharedPreferences 封装
│   │   │   ├── hive_storage.dart         # Hive 封装
│   │   │   └── storage_keys.dart         # 存储 Key 常量
│   │   │
│   │   ├── audio/                         # 音频服务
│   │   │   ├── audio_service.dart        # 音频播放服务
│   │   │   ├── piano_sound.dart          # 钢琴音色
│   │   │   └── metronome_service.dart    # 节拍器服务
│   │   │
│   │   ├── theme/                         # 主题系统
│   │   │   ├── app_theme.dart            # 主题配置
│   │   │   ├── app_colors.dart           # 颜色定义
│   │   │   ├── app_text_styles.dart      # 文字样式
│   │   │   ├── app_dimensions.dart       # 尺寸定义
│   │   │   └── theme_controller.dart     # 主题控制器
│   │   │
│   │   ├── utils/                         # 工具类
│   │   │   ├── logger.dart               # 日志工具
│   │   │   ├── validators.dart           # 验证工具
│   │   │   ├── date_utils.dart           # 日期工具
│   │   │   └── music_utils.dart          # 音乐工具（音高计算等）
│   │   │
│   │   └── widgets/                       # 通用组件
│   │       ├── buttons/                  # 按钮组件
│   │       │   └── app_button.dart
│   │       ├── cards/                    # 卡片组件
│   │       │   └── app_card.dart
│   │       ├── feedback/                 # 反馈组件
│   │       │   ├── app_loading.dart
│   │       │   ├── app_empty.dart
│   │       │   ├── app_error.dart
│   │       │   └── app_toast.dart
│   │       ├── inputs/                   # 输入组件
│   │       │   └── app_text_field.dart
│   │       ├── music/                    # 音乐专用组件
│   │       │   ├── staff_widget.dart    # 五线谱组件
│   │       │   ├── piano_keyboard.dart  # 钢琴键盘组件
│   │       │   ├── note_widget.dart     # 音符组件
│   │       │   └── rhythm_widget.dart   # 节奏组件
│   │       └── layout/                   # 布局组件
│   │           ├── app_scaffold.dart
│   │           └── responsive_builder.dart
│   │
│   ├── features/                           # 🎯 功能模块层（按业务划分）
│   │   │
│   │   ├── splash/                        # 启动页模块
│   │   │   └── views/
│   │   │       └── splash_page.dart
│   │   │
│   │   ├── onboarding/                    # 引导页模块
│   │   │   ├── controllers/
│   │   │   │   └── onboarding_controller.dart
│   │   │   └── views/
│   │   │       └── onboarding_page.dart
│   │   │
│   │   ├── home/                          # 首页模块
│   │   │   ├── bindings/
│   │   │   │   └── home_binding.dart
│   │   │   ├── controllers/
│   │   │   │   └── home_controller.dart
│   │   │   ├── views/
│   │   │   │   ├── home_page.dart
│   │   │   │   └── widgets/
│   │   │   │       ├── learning_map_widget.dart
│   │   │   │       ├── daily_task_widget.dart
│   │   │   │       └── continue_learning_widget.dart
│   │   │   └── models/
│   │   │       └── home_data_model.dart
│   │   │
│   │   ├── course/                        # 课程模块
│   │   │   ├── bindings/
│   │   │   │   └── course_binding.dart
│   │   │   ├── controllers/
│   │   │   │   ├── course_list_controller.dart
│   │   │   │   └── course_detail_controller.dart
│   │   │   ├── models/
│   │   │   │   ├── course_model.dart
│   │   │   │   ├── lesson_model.dart
│   │   │   │   └── course_progress_model.dart
│   │   │   ├── repositories/
│   │   │   │   ├── course_repository.dart
│   │   │   │   └── course_repository_impl.dart
│   │   │   ├── services/
│   │   │   │   └── course_service.dart
│   │   │   └── views/
│   │   │       ├── course_list_page.dart
│   │   │       ├── course_detail_page.dart
│   │   │       ├── lesson_page.dart
│   │   │       └── widgets/
│   │   │           ├── course_card.dart
│   │   │           └── lesson_item.dart
│   │   │
│   │   ├── practice/                      # 练习模块
│   │   │   ├── bindings/
│   │   │   │   └── practice_binding.dart
│   │   │   ├── controllers/
│   │   │   │   ├── practice_controller.dart
│   │   │   │   ├── note_practice_controller.dart
│   │   │   │   ├── rhythm_practice_controller.dart
│   │   │   │   └── piano_practice_controller.dart
│   │   │   ├── models/
│   │   │   │   ├── practice_question_model.dart
│   │   │   │   └── practice_record_model.dart
│   │   │   ├── repositories/
│   │   │   │   └── practice_repository.dart
│   │   │   ├── services/
│   │   │   │   ├── question_generator.dart
│   │   │   │   └── practice_evaluator.dart
│   │   │   └── views/
│   │   │       ├── practice_home_page.dart
│   │   │       ├── note_practice_page.dart
│   │   │       ├── rhythm_practice_page.dart
│   │   │       ├── piano_practice_page.dart
│   │   │       └── practice_result_page.dart
│   │   │
│   │   ├── tools/                         # 工具模块
│   │   │   ├── piano/                    # 虚拟钢琴
│   │   │   │   ├── controllers/
│   │   │   │   │   └── piano_controller.dart
│   │   │   │   └── views/
│   │   │   │       └── piano_page.dart
│   │   │   ├── metronome/                # 节拍器
│   │   │   │   ├── controllers/
│   │   │   │   │   └── metronome_controller.dart
│   │   │   │   └── views/
│   │   │   │       └── metronome_page.dart
│   │   │   └── sheet_music/              # 乐谱库
│   │   │       ├── controllers/
│   │   │       ├── models/
│   │   │       └── views/
│   │   │
│   │   └── profile/                       # 个人中心模块
│   │       ├── bindings/
│   │       │   └── profile_binding.dart
│   │       ├── controllers/
│   │       │   ├── profile_controller.dart
│   │       │   ├── statistics_controller.dart
│   │       │   └── settings_controller.dart
│   │       ├── models/
│   │       │   ├── user_model.dart
│   │       │   ├── achievement_model.dart
│   │       │   └── statistics_model.dart
│   │       ├── repositories/
│   │       │   └── user_repository.dart
│   │       └── views/
│   │           ├── profile_page.dart
│   │           ├── statistics_page.dart
│   │           ├── achievement_page.dart
│   │           └── settings_page.dart
│   │
│   └── shared/                             # 📦 共享资源
│       ├── constants/                     # 常量定义
│       │   ├── music_constants.dart      # 音乐相关常量
│       │   ├── app_constants.dart        # 应用常量
│       │   └── asset_paths.dart          # 资源路径
│       │
│       ├── extensions/                    # Dart 扩展
│       │   ├── string_ext.dart
│       │   ├── context_ext.dart
│       │   ├── list_ext.dart
│       │   └── datetime_ext.dart
│       │
│       ├── enums/                         # 枚举定义
│       │   ├── practice_type.dart
│       │   ├── note_type.dart
│       │   └── difficulty_level.dart
│       │
│       └── mixins/                        # Mixin
│           ├── loading_mixin.dart
│           └── toast_mixin.dart
│
├── assets/                                 # 📁 静态资源
│   ├── images/                            # 图片
│   │   ├── icons/                        # 图标
│   │   ├── illustrations/                # 插图
│   │   └── instruments/                  # 乐器图
│   ├── audio/                             # 音频
│   │   ├── piano/                        # 钢琴音（按音符命名）
│   │   ├── metronome/                    # 节拍器音效
│   │   └── effects/                      # 效果音
│   ├── fonts/                             # 字体
│   ├── data/                              # 数据文件
│   │   ├── courses/                      # 课程数据 JSON
│   │   ├── questions/                    # 练习题库 JSON
│   │   └── sheets/                       # 乐谱数据 JSON
│   └── animations/                        # Lottie 动画
│
├── test/                                   # 🧪 测试
│   ├── unit/                              # 单元测试
│   ├── widget/                            # Widget 测试
│   └── integration/                       # 集成测试
│
├── docs/                                   # 📄 文档
│   ├── 架构设计文档.md
│   ├── 产品设计文档.md
│   └── 应用架构设计文档.md
│
├── pubspec.yaml                            # 依赖配置
├── analysis_options.yaml                   # 代码分析规则
└── README.md                               # 项目说明
```

---

## 四、核心架构设计

### 4.1 依赖注入（DI）

使用 GetX 的依赖注入系统管理全局服务：

```dart
// app/app_binding.dart
class AppBinding extends Bindings {
  @override
  void dependencies() {
    // 核心服务（永久保持）
    Get.put<StorageService>(StorageServiceImpl(), permanent: true);
    Get.put<HttpClient>(HttpClient(), permanent: true);
    Get.put<AudioService>(AudioServiceImpl(), permanent: true);
    
    // 主题控制器
    Get.put<ThemeController>(ThemeController(), permanent: true);
    
    // Repository（懒加载）
    Get.lazyPut<CourseRepository>(() => CourseRepositoryImpl());
    Get.lazyPut<PracticeRepository>(() => PracticeRepositoryImpl());
    Get.lazyPut<UserRepository>(() => UserRepositoryImpl());
  }
}
```

### 4.2 Repository 模式

```
┌─────────────────────────────────────────────────────────────────┐
│                        Controller                                │
│                           │                                      │
│                           ▼                                      │
│              ┌────────────────────────┐                         │
│              │      Repository        │  ← 接口定义              │
│              │      (Abstract)        │                         │
│              └───────────┬────────────┘                         │
│                          │                                       │
│           ┌──────────────┼──────────────┐                       │
│           ▼              ▼              ▼                       │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐            │
│  │ Remote Impl  │ │  Local Impl  │ │  Mock Impl   │            │
│  │  (网络实现)   │ │  (本地实现)   │ │  (测试实现)  │            │
│  └──────────────┘ └──────────────┘ └──────────────┘            │
└─────────────────────────────────────────────────────────────────┘
```

```dart
// 接口定义
abstract class CourseRepository {
  Future<List<Course>> getCourses();
  Future<Course> getCourseDetail(String id);
  Future<void> updateProgress(String courseId, double progress);
}

// 实现
class CourseRepositoryImpl implements CourseRepository {
  final HttpClient _http;
  final StorageService _storage;
  
  CourseRepositoryImpl({
    HttpClient? http,
    StorageService? storage,
  }) : _http = http ?? Get.find(),
       _storage = storage ?? Get.find();
  
  @override
  Future<List<Course>> getCourses() async {
    // 先查本地缓存
    final cached = await _storage.get<List<Course>>('courses');
    if (cached != null) return cached;
    
    // 本地无数据，从 assets 加载
    final data = await rootBundle.loadString('assets/data/courses/list.json');
    final courses = (jsonDecode(data) as List)
        .map((e) => Course.fromJson(e))
        .toList();
    
    // 缓存到本地
    await _storage.set('courses', courses);
    return courses;
  }
  
  // ...
}
```

### 4.3 状态管理模式

使用 GetX 响应式状态管理：

```dart
// Controller 基类
abstract class BaseController extends GetxController {
  // 通用状态
  final isLoading = false.obs;
  final errorMessage = ''.obs;
  
  // 状态判断
  bool get hasError => errorMessage.isNotEmpty;
  
  // 通用加载处理
  Future<T?> handleLoading<T>(Future<T> Function() action) async {
    try {
      isLoading.value = true;
      errorMessage.value = '';
      return await action();
    } catch (e) {
      errorMessage.value = e.toString();
      return null;
    } finally {
      isLoading.value = false;
    }
  }
}

// 具体 Controller
class CourseListController extends BaseController {
  final CourseRepository _repository;
  
  CourseListController({CourseRepository? repository})
      : _repository = repository ?? Get.find();
  
  final courses = <Course>[].obs;
  final selectedCategory = 'all'.obs;
  
  // 筛选后的课程
  List<Course> get filteredCourses {
    if (selectedCategory.value == 'all') return courses;
    return courses.where((c) => c.categoryId == selectedCategory.value).toList();
  }
  
  @override
  void onInit() {
    super.onInit();
    loadCourses();
  }
  
  Future<void> loadCourses() async {
    final result = await handleLoading(() => _repository.getCourses());
    if (result != null) {
      courses.value = result;
    }
  }
  
  void selectCategory(String categoryId) {
    selectedCategory.value = categoryId;
  }
}
```

### 4.4 页面状态组件

```dart
// 统一的页面状态处理
class StateView<T extends BaseController> extends GetView<T> {
  final Widget Function(T controller) builder;
  final Widget? loadingWidget;
  final Widget Function(String error)? errorBuilder;
  
  const StateView({
    required this.builder,
    this.loadingWidget,
    this.errorBuilder,
  });
  
  @override
  Widget build(BuildContext context) {
    return Obx(() {
      // 加载中
      if (controller.isLoading.value) {
        return loadingWidget ?? const AppLoading();
      }
      
      // 错误状态
      if (controller.hasError) {
        return errorBuilder?.call(controller.errorMessage.value)
            ?? AppError(
                 message: controller.errorMessage.value,
                 onRetry: () => controller.onInit(),
               );
      }
      
      // 正常内容
      return builder(controller);
    });
  }
}
```

---

## 五、模块通信设计

### 5.1 事件总线

```dart
// 定义事件
class LearningProgressEvent {
  final String courseId;
  final double progress;
  
  LearningProgressEvent(this.courseId, this.progress);
}

class PracticeCompletedEvent {
  final String practiceType;
  final int score;
  
  PracticeCompletedEvent(this.practiceType, this.score);
}

// 使用（基于 GetX 的 ever/once）
class HomeController extends BaseController {
  @override
  void onInit() {
    super.onInit();
    
    // 监听学习进度变化
    ever(Get.find<CourseController>().currentProgress, (progress) {
      refreshLearningMap();
    });
  }
}
```

### 5.2 模块间依赖关系

```
┌─────────────────────────────────────────────────────────────────┐
│                          模块依赖关系图                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│                      ┌─────────────┐                            │
│                      │    Core     │                            │
│                      │   核心层    │                            │
│                      └──────┬──────┘                            │
│                             │                                    │
│           ┌─────────────────┼─────────────────┐                 │
│           ▼                 ▼                 ▼                 │
│   ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│   │    Course    │  │   Practice   │  │   Profile    │         │
│   │    课程模块   │  │   练习模块    │  │  个人中心    │         │
│   └──────┬───────┘  └──────┬───────┘  └──────┬───────┘         │
│          │                 │                 │                  │
│          └─────────────────┼─────────────────┘                  │
│                            ▼                                     │
│                    ┌──────────────┐                             │
│                    │     Home     │                             │
│                    │    首页模块   │                             │
│                    └──────────────┘                             │
│                                                                 │
│  依赖规则：                                                      │
│  1. 所有模块可依赖 Core 层                                        │
│  2. 模块间通过 Repository/Service 接口通信                        │
│  3. Home 模块聚合其他模块数据                                     │
│  4. 禁止循环依赖                                                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 六、路由设计

### 6.1 路由命名规范

```dart
// app/routes/app_routes.dart
abstract class AppRoutes {
  // 基础页面
  static const splash = '/splash';
  static const onboarding = '/onboarding';
  static const main = '/main';
  
  // 课程模块
  static const courseList = '/course/list';
  static const courseDetail = '/course/detail';
  static const lesson = '/course/lesson';
  
  // 练习模块
  static const practiceHome = '/practice';
  static const notePractice = '/practice/note';
  static const rhythmPractice = '/practice/rhythm';
  static const pianoPractice = '/practice/piano';
  static const practiceResult = '/practice/result';
  
  // 工具模块
  static const piano = '/tools/piano';
  static const metronome = '/tools/metronome';
  static const sheetMusic = '/tools/sheet-music';
  
  // 个人中心
  static const profile = '/profile';
  static const statistics = '/profile/statistics';
  static const achievements = '/profile/achievements';
  static const settings = '/profile/settings';
}
```

### 6.2 路由配置

```dart
// app/routes/app_pages.dart
class AppPages {
  static final pages = [
    // 启动页
    GetPage(
      name: AppRoutes.splash,
      page: () => const SplashPage(),
    ),
    
    // 引导页
    GetPage(
      name: AppRoutes.onboarding,
      page: () => const OnboardingPage(),
      binding: OnboardingBinding(),
    ),
    
    // 主页（带底部导航）
    GetPage(
      name: AppRoutes.main,
      page: () => const MainPage(),
      binding: MainBinding(),
    ),
    
    // 课程详情
    GetPage(
      name: AppRoutes.courseDetail,
      page: () => const CourseDetailPage(),
      binding: CourseDetailBinding(),
      transition: Transition.rightToLeft,
    ),
    
    // 练习页（需要登录/引导完成）
    GetPage(
      name: AppRoutes.notePractice,
      page: () => const NotePracticePage(),
      binding: NotePracticeBinding(),
      middlewares: [OnboardingMiddleware()],
    ),
    
    // ...更多路由
  ];
}
```

### 6.3 路由中间件

```dart
// 引导完成检查
class OnboardingMiddleware extends GetMiddleware {
  @override
  RouteSettings? redirect(String? route) {
    final storage = Get.find<StorageService>();
    final isOnboardingCompleted = storage.getBool(StorageKeys.onboardingCompleted) ?? false;
    
    if (!isOnboardingCompleted) {
      return const RouteSettings(name: AppRoutes.onboarding);
    }
    return null;
  }
}
```

---

## 七、数据流设计

### 7.1 数据流向图

```
┌─────────────────────────────────────────────────────────────────┐
│                         数据流向                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐ │
│  │   UI     │───▶│Controller│───▶│Repository│───▶│  Data    │ │
│  │  用户操作 │    │  业务逻辑 │    │  数据获取 │    │  数据源   │ │
│  └──────────┘    └──────────┘    └──────────┘    └──────────┘ │
│       ▲                │                               │        │
│       │                │                               │        │
│       │                ▼                               ▼        │
│       │         ┌──────────┐                   ┌──────────┐    │
│       │         │  State   │                   │  Assets  │    │
│       └─────────│  状态更新 │                   │  JSON    │    │
│                 └──────────┘                   └──────────┘    │
│                                                         │        │
│                                                         ▼        │
│                                                  ┌──────────┐   │
│                                                  │   Hive   │   │
│                                                  │  本地存储 │   │
│                                                  └──────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.2 数据缓存策略

```dart
// 缓存策略枚举
enum CacheStrategy {
  networkFirst,    // 优先网络，失败用缓存
  cacheFirst,      // 优先缓存，过期走网络
  networkOnly,     // 仅网络
  cacheOnly,       // 仅缓存
}

// 带缓存的 Repository
class CachedRepository<T> {
  final StorageService _storage;
  final String cacheKey;
  final Duration cacheDuration;
  
  Future<T?> getData({
    required Future<T> Function() fetcher,
    CacheStrategy strategy = CacheStrategy.cacheFirst,
  }) async {
    switch (strategy) {
      case CacheStrategy.cacheFirst:
        final cached = await _getCached();
        if (cached != null && !_isExpired()) {
          return cached;
        }
        final fresh = await fetcher();
        await _saveCache(fresh);
        return fresh;
        
      case CacheStrategy.networkFirst:
        try {
          final fresh = await fetcher();
          await _saveCache(fresh);
          return fresh;
        } catch (e) {
          return await _getCached();
        }
        
      // ...其他策略
    }
  }
}
```

---

## 八、音频系统设计

### 8.1 音频服务架构

```
┌─────────────────────────────────────────────────────────────────┐
│                        音频系统架构                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                     AudioService                          │  │
│  │                     (音频服务接口)                         │  │
│  └─────────────────────────┬────────────────────────────────┘  │
│                            │                                    │
│           ┌────────────────┼────────────────┐                  │
│           ▼                ▼                ▼                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │ PianoSound   │  │ Metronome    │  │ EffectSound  │         │
│  │ 钢琴音色     │  │ 节拍器       │  │ 效果音       │         │
│  └──────────────┘  └──────────────┘  └──────────────┘         │
│         │                 │                 │                  │
│         ▼                 ▼                 ▼                  │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │                    AudioPlayer Pool                      │  │
│  │                    (音频播放器池)                         │  │
│  │   支持多音同时播放（和弦）                                 │  │
│  └─────────────────────────────────────────────────────────┘  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 8.2 钢琴音色管理

```dart
// 音频服务
class PianoSoundService {
  // 音频池（支持同时播放多个音）
  final Map<int, AudioPlayer> _playerPool = {};
  
  // 预加载音频
  Future<void> preloadSounds() async {
    // 预加载常用音域 (C3-C6)
    for (int midi = 48; midi <= 84; midi++) {
      final player = AudioPlayer();
      await player.setAsset('assets/audio/piano/note_$midi.mp3');
      _playerPool[midi] = player;
    }
  }
  
  // 播放音符
  Future<void> playNote(int midiNumber) async {
    final player = _playerPool[midiNumber];
    if (player != null) {
      await player.seek(Duration.zero);
      await player.play();
    }
  }
  
  // 停止音符
  Future<void> stopNote(int midiNumber) async {
    final player = _playerPool[midiNumber];
    await player?.stop();
  }
  
  // 播放和弦
  Future<void> playChord(List<int> midiNumbers) async {
    await Future.wait(midiNumbers.map((n) => playNote(n)));
  }
}
```

---

## 九、五线谱渲染设计

### 9.1 渲染架构

```
┌─────────────────────────────────────────────────────────────────┐
│                      五线谱渲染系统                               │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │                    StaffWidget                            │  │
│  │                   (五线谱组件)                             │  │
│  │                                                           │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐       │  │
│  │  │  ClefPainter │  │ StaffPainter│  │ NotePainter │       │  │
│  │  │  谱号绘制    │  │  五线绘制   │  │  音符绘制   │       │  │
│  │  └─────────────┘  └─────────────┘  └─────────────┘       │  │
│  │                                                           │  │
│  │  ┌─────────────┐  ┌─────────────┐                        │  │
│  │  │ AccidentalP │  │  RestPainter │                        │  │
│  │  │  升降号绘制  │  │  休止符绘制  │                        │  │
│  │  └─────────────┘  └─────────────┘                        │  │
│  │                                                           │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                 │
│  数据模型：                                                      │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  StaffData {                                              │  │
│  │    clef: Clef.treble,                                     │  │
│  │    keySignature: KeySignature.cMajor,                     │  │
│  │    timeSignature: TimeSignature(4, 4),                    │  │
│  │    notes: [NoteData(...)],                                │  │
│  │  }                                                        │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 9.2 音符位置计算

```dart
// 五线谱配置
class StaffConfig {
  final double lineSpacing;      // 线间距
  final double noteRadius;       // 音符半径
  final double staffWidth;       // 五线谱宽度
  
  // 根据 MIDI 编号计算 Y 位置
  double getYPosition(int midiNumber, Clef clef) {
    // 中央 C (MIDI 60) 在高音谱表下加一线
    // 每个半音对应特定位置
    final positions = _getNotePositions(clef);
    final position = positions[midiNumber % 12];
    final octaveOffset = (midiNumber ~/ 12 - 5) * 7 * lineSpacing / 2;
    
    return baseY - position * lineSpacing / 2 - octaveOffset;
  }
}
```

---

## 十、错误处理设计

### 10.1 异常层级

```dart
// 基础异常
abstract class AppException implements Exception {
  final String message;
  final String? code;
  
  const AppException(this.message, {this.code});
}

// 网络异常
class NetworkException extends AppException {
  const NetworkException(super.message, {super.code});
}

// 业务异常
class BusinessException extends AppException {
  const BusinessException(super.message, {super.code});
}

// 存储异常
class StorageException extends AppException {
  const StorageException(super.message, {super.code});
}

// 音频异常
class AudioException extends AppException {
  const AudioException(super.message, {super.code});
}
```

### 10.2 全局错误处理

```dart
// 错误处理工具
class ErrorHandler {
  static void handle(dynamic error, {StackTrace? stackTrace}) {
    // 日志记录
    Logger.error('Error: $error', stackTrace: stackTrace);
    
    // 用户提示
    String message;
    if (error is NetworkException) {
      message = '网络异常，请检查网络连接';
    } else if (error is BusinessException) {
      message = error.message;
    } else {
      message = '出了点问题，请稍后重试';
    }
    
    Get.snackbar('提示', message);
  }
}
```

---

## 十一、测试策略

### 11.1 测试金字塔

```
                    ┌───────────┐
                    │   E2E     │  端到端测试（少量）
                    │   Tests   │  关键流程
                   ┌┴───────────┴┐
                   │   Widget    │  组件测试（中等）
                   │   Tests     │  UI 交互
                  ┌┴─────────────┴┐
                  │     Unit      │  单元测试（大量）
                  │     Tests     │  Service, Repository, Utils
                 ─┴───────────────┴─
```

### 11.2 测试示例

```dart
// 单元测试：课程 Repository
void main() {
  group('CourseRepository', () {
    late CourseRepository repository;
    late MockStorageService mockStorage;
    
    setUp(() {
      mockStorage = MockStorageService();
      repository = CourseRepositoryImpl(storage: mockStorage);
    });
    
    test('getCourses returns cached data when available', () async {
      // Arrange
      when(mockStorage.get('courses')).thenAnswer((_) async => mockCourses);
      
      // Act
      final result = await repository.getCourses();
      
      // Assert
      expect(result, equals(mockCourses));
      verify(mockStorage.get('courses')).called(1);
    });
  });
}
```

---

## 十二、性能优化

### 12.1 优化策略

| 场景 | 优化方案 |
|------|---------|
| **列表渲染** | ListView.builder 懒加载 |
| **图片加载** | cached_network_image 缓存 |
| **音频预加载** | 常用音域提前加载到内存 |
| **五线谱渲染** | RepaintBoundary 隔离重绘 |
| **状态更新** | 细粒度 Obx，避免大范围刷新 |
| **页面切换** | 合理使用 const 构造函数 |

### 12.2 内存管理

```dart
// Controller 生命周期管理
class PracticeController extends BaseController {
  late AudioService _audioService;
  
  @override
  void onInit() {
    super.onInit();
    _audioService = Get.find();
    _audioService.preloadSounds();  // 预加载
  }
  
  @override
  void onClose() {
    _audioService.dispose();  // 释放资源
    super.onClose();
  }
}
```

---

## 十三、代码规范

### 13.1 文件命名

| 类型 | 命名规则 | 示例 |
|------|---------|------|
| 文件 | 小写 + 下划线 | `course_controller.dart` |
| 类 | 大驼峰 | `CourseController` |
| 变量 | 小驼峰 | `courseList` |
| 常量 | 小驼峰 | `defaultPageSize` |
| 私有 | 下划线前缀 | `_isLoading` |

### 13.2 目录规范

```
每个功能模块目录结构：
feature_name/
├── bindings/           # 依赖绑定（1个文件）
├── controllers/        # 控制器（可多个）
├── models/            # 数据模型
├── repositories/      # 数据仓储（接口+实现）
├── services/          # 业务服务
└── views/             # 页面和组件
    ├── xxx_page.dart
    └── widgets/       # 页面私有组件
```

### 13.3 注释规范

```dart
/// 课程控制器
/// 
/// 负责课程列表的加载、筛选和状态管理
class CourseController extends BaseController {
  /// 加载课程列表
  /// 
  /// [forceRefresh] 是否强制刷新（忽略缓存）
  Future<void> loadCourses({bool forceRefresh = false}) async {
    // ...
  }
}
```

---

## 十四、文档清单

| 文档 | 内容 | 状态 |
|------|------|------|
| 架构设计文档.md | 脚手架架构（通用） | ✅ 完成 |
| 产品设计文档.md | 乐理通 App 产品设计 | ✅ 完成 |
| 应用架构设计文档.md | 乐理通 App 技术架构 | ✅ 完成 |
| API 文档 | 接口定义（如需后端） | 📝 待定 |
| 组件文档 | 通用组件使用说明 | 📝 开发时补充 |

---

*文档版本：v1.0*  
*更新时间：2024年*

